...
df.stat <-
  ggpubr::compare_means(Expression ~ Sample_Cluster, 
                      ...
                      method = "wilcox.test", 
                      p.adjust.method = "bonferroni") %>%
  dplyr::rename(`Sample Cluster` = group2,
...
df <- left_join(df, df.meta, by = "SAMPLE_ID")

filter(df, `Gene Cluster` == "G1") %>%
  coxph(formula = Surv(OS_MONTHS, as.numeric(as.factor(OS_STATUS))) ~ Expression, data = .) %>%
  summary()

filter(df, `Gene Cluster` == "G1") %>%
  coxph(formula = Surv(PFS_MONTHS, as.numeric(as.factor(PFS_STATUS))) ~ Expression, data = .) %>%
  summary()
